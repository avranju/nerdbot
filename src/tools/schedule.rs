...
impl Tool for ScheduleJob {
    ...
    async fn execute(
        ...
        let name = args
            .get("name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| AgentError::Generic("name is required".into()))?
            .to_string();
        let prompt = args
            .get("prompt")
            .and_then(|v| v.as_str())
            .ok_or_else(|| AgentError::Generic("prompt is required".into()))?
            .to_string();
        let schedule_type_str = args
            .get("schedule_type")
            .and_then(|v| v.as_str())
            .ok_or_else(|| AgentError::Generic("schedule_type is required".into()))?;
        let timezone = args
            .get("timezone")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        let notify_on_completion = args
            .get("notify_on_completion")
            .and_then(|v| v.as_bool())
            .unwrap_or(true);
        let context_policy_str = args
            .get("context_policy")
            .and_then(|v| v.as_str())
            .unwrap_or("include_creation_snapshot");
        ...
        let context_policy = match context_policy_str {
            ...
            "include_chat_summary" => {
                crate::scheduler::models::JobContextPolicy::IncludeChatSummary
            }
            _ => {
                return Err(AgentError::Generic(format!(
                    "Invalid context_policy '{}'",
                    context_policy_str
                )));
            }
        };
        ...
        let pool = match ctx.pool {
            ...
            None => {
                return Ok(ToolOutput {
                    ...
                    summary: "Scheduler not configured in this context, returned mock job creation"
                        .to_string(),
                });
            }
        };
        ...
        match schedule_type {
            crate::scheduler::models::ScheduleType::OneShot => {
                let run_at_str = args.get("run_at").and_then(|v| v.as_str()).ok_or_else(|| {
                    AgentError::Generic("run_at is required for one_shot job".into())
                })?;
                let run_at_parsed = chrono::DateTime::parse_from_rfc3339(run_at_str)
                    .map_err(|e| {
                        AgentError::Generic(format!(
                            "Invalid ISO-8601 date-time '{}': {e}",
                            run_at_str
                        ))
                    })?
            ...
            crate::scheduler::models::ScheduleType::Cron => {
                let cron_str = args
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        AgentError::Generic("cron_expression is required for cron job".into())
                    })?;
        ...
        let creation_context_snapshot = if context_policy
            == crate::scheduler::models::JobContextPolicy::IncludeCreationSnapshot
        {
            if let Some(session) =
        ...
        Ok(ToolOutput {
            success: true,
            summary: format!(
                "Successfully scheduled job \"{}\" with ID `{}`.",
                name, job.id
            ),
        ...
        let pool = match ctx.pool {
            ...
            None => {
                return Ok(ToolOutput {
                    success: true,
                    data: json!([]),
                    summary: "Scheduler not configured in this context, returned empty list"
                        .to_string(),
                });
            }
        };
...
impl Tool for DeleteJob {
    ...
    async fn execute(
        ...
        let job_id = args
            .get("job_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| AgentError::Generic("job_id is required".into()))?;
        ...
        let pool = match ctx.pool {
            ...
            None => {
                return Ok(ToolOutput {
                    ...
                    summary: format!(
                        "Scheduler not configured in this context, mock deleted job `{}`",
                        job_id
                    ),
                });
            }
        ...
        let pool = match ctx.pool {
            ...
            None => {
                return Ok(ToolOutput {
                    ...
                    data: json!({ "job_id": job_id }),
                    summary: format!(
                        "Scheduler not configured in this context, mock triggered job `{}`",
                        job_id
                    ),
                });
            }
        ...
        match job {
            ...
            Some(_) => {
                ...
                Ok(ToolOutput {
                    success: true,
                    summary: format!(
                        "Job `{}` successfully scheduled to run immediately.",
                        job_id
                    ),
                    data: json!({ "job_id": job_id }),
                })
